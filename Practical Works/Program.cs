...
namespace Practical_Works
{
    class Program
    {
        public static List<(string Name, PracticalWork MainClass)> practicalWorks = new()
        {
            ...
            #endregion
            #region Классы 2
            ("Классы 2", new PracticalWork(("Задание 1", "1 Создать класс счет в банке с закрытыми полями: номер счета, баланс. Тип банковского счета (текущий). Предусмотреть методы для доступа к данным – заполнения и чтения. Создать объект класса, заполнить его поля и вывести информацию об объекте класса на печать.\n2 Изменить класс счет в банке из упражнения 1 таким образом, чтобы номер счета генерировался сам и был уникальным.Для этого надо создать в классе статическую переменную и метод, который увеличивает значение этой переменной.\n3 Добавить в класс счет в банке два метода: снять со счета и положить на счет.Метод снять со счета проверяет, возможно ли снять запрашиваемую сумму, и в случае положительного результата изменяет баланс.", Classes2.Task1),
                                           ("Задание 2", "Реализовать класс для описания здания (уникальный номер здания, высота, этажность, количество квартир, подъездов). Поля сделать закрытыми, предусмотреть методы для заполнения полей и получения значений полей для печати. \nДобавить методы вычисления высоты этажа, количества квартир в подъезде, количества квартир на этаже и т.д.\nПредусмотреть возможность, чтобы уникальный номер здания генерировался программно.Для этого в классе предусмотреть статическое поле, которое бы хранило последний использованный номер здания, и предусмотреть метод, который увеличивал бы значение этого поля.", Classes2.Task2)))
            #endregion
        };
